...
module.exports = {
    name: 'jp-profile',
    description: "View a saved FGO JP profile (yours or another player's).",
    type: 1,
    options: [
    ...
    run: async (client, interaction, config, db) => {
        try {
            ...
            const raw = await db.get(`fgoProfile_Jp_${target.id}`);
            if (!raw) {
                return interaction.reply({
                    content: isOther
                        ? 'Cannot find a profile for that player.'
                        : 'You have no JP profile yet. Use `/jp-profile-edit` to create one.',
                    ephemeral: true,
                });
            }
            ...
            util.fgoProfiles(target, data, interaction, config.ProfileView.TIMEOUT, false, PAGE_COUNT);
        } catch (err) {
